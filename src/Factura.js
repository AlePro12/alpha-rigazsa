const Factura = {
  ...
  precioDecimals: 2,
  cantidadDecimals: 2,
  init(Impresora, { rif, razonSocial }) {
    this.rif = rif;
  ...
  Open: async function () {
    ...
    cmd.push(SEP);
    cmd = cmd.concat(this.Impresora.stringToDecimalArray(' ')); //NumFactuDev
    cmd.push(SEP);
    ...
    console.log('🚀 ~ file: Factura.js ~ line 50 ~ Open ~ result', result);
    return result;
  },
  ...
  AddDiscountPercent: async function ({ percent }) {
    console.trace(
      '****** EL DESCUENTO NO ESTA IMPLEMENTADO POR LOS MOMENTOS YA QUE EN ESTA IMPRESORA LOS DESCUENTOS SON GLOBALES ES DECIR NO ES ITEM POR ITEM ASI QUE ESTA FUNCION NO TENDRIA SENTIDO EN ESTE CONTEXTO ******'
    );
  },
  AddCode(data) {},
  Pago({ tipo, monto }) {},
